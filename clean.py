...
epa_ind = (
    after[after["type"] != "汰舊"].groupby(by=["time", "city", "type"])["epa_app"].sum()
)
...
epa_r = (
    epa_ind.div(epa_ind.groupby(["time", "city"]).transform("sum"))
    .reset_index()
...
after = epa_m.merge(
    moea[["city", "time", "moea_app"]], how="left", on=["time", "city"], indicator=True
)
...
after = after.drop(columns=["_merge", "epa_ratio", "moea_app_alloc"]).merge(
